...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('instrumentChanged', ({ detail: { row, type } }) => {
    // 1) update the name & rebuild voices
    instrumentNames[row] = type;
    voices = createVoices(instrumentNames);

    // 2) capture play state & position
    const wasPlaying = Tone.Transport.state === 'started';
    const currentPos = Tone.Transport.position;

    // 3) clear old schedule and re-init
    Tone.Transport.cancel(); 
    initSequencer(voices, Number(bpmIn.value));

    // 4) if user was playing, restart from where they were
    if (wasPlaying) {
      Tone.Transport.start(undefined, currentPos);
    }
  });
});
