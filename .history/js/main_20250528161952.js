...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 8) Instrument dropdown change → swap that row’s voice on the fly
  document.addEventListener('instrumentChanged', ({ detail: { row, type } }) => {
    // 1) Update the instrument name & rebuild all voices[]
    instrumentNames[row] = type;
    voices = createVoices(instrumentNames);

    // 2) Remember if it was playing and where
    const wasPlaying = Tone.Transport.state === 'started';
    const position   = Tone.Transport.position; // "bars:beats:sixteenths"

    // 3) Clear out old schedule and set up the new one
    Tone.Transport.stop();
    Tone.Transport.cancel(); 
    initSequencer(voices, Number(bpmIn.value));

    // 4) If it was playing, restart from the same position
    if (wasPlaying) {
      Tone.Transport.start(undefined, position);
    }
  }); 
});
